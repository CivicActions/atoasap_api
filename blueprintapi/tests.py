...
class UnauthenticatedAPITestCase(APITestCase):
    ...
    def test_expired_token_is_refreshed_on_successful_auth(self):
        ...
        create_user = self.client.post(
            "/api/users/", data=login, content_type="application/json"
        )
        self.assertEqual(create_user.status_code, status.HTTP_201_CREATED)

        initial = self.client.post(
            "/api-token-auth/", data=login, content_type="application/json"
        )
                        ...
                        "implemented-requirements": [
                            {
                                ...
                                "control-id": "ac-3",
                                "description": (
                                    "The Django web framework implements role-based access control to "
                                    "enforce logical access to its information and services."
                                ),
                                "props": [
                                    {
                            ...
                            },
                            {
                                ...
                                "control-id": "ac-7",
                                "description": (
                                    "Django can be configured to lock an account after a specified "
                                    "number of invalid login attempts within a specified time period."
                                    " These values have been configured in accordance with CMS ARS "
                                    "guidance to lock the account automatically after three (3) "
                            ...
                            },
                            {
                                ...
                                "control-id": "ac-11",
                                "description": (
                                    "The Django web framework has built-in session management that "
                                    "includes locking and terminating a session after a specific "
                                    "duration of inactivity. The duration of a session is set by "
                                    "configuring the `SESSION_COOKIE_AGE` in `settings.py`."
                                ),
                                "props": [
                                    {
                                        "name": "security_control_type",
...
class ComponentModelTestCase(SimpleTestCase):
    def test_from_json(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            component = ComponentModel.from_json(path)

            # Test a couple of items to confirm content.
            self.assertEqual(
                component.component_definition.metadata.title, "TestComponent"
            )
            self.assertEqual(len(component.component_definition.components), 1)
            self.assertEqual(
                str(component.component_definition.components[0].uuid),
                "d42844ee-50a3-40f5-8a02-ccc351e2a95a",
            )

    def test_control_ids(self):
        component = ComponentModel(**COMPONENT_DATA)
        self.assertEqual(
            sorted(component.component_definition.components[0].control_ids),
            ["ac-11", "ac-3", "ac-7"],
        )
